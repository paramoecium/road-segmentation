...
def mainFunc(argv):
    ...
    print("Shape of targets from dlm: {}".format(targets.shape)) #
    ...
    with tf.Session(config=configProto) as sess:
        ...
        if conf.run_on_test_set:
            ...
            targets = extract_data(prediction_test_dir, patches_per_image_test, conf.test_size)
            ...
            for i in range(conf.test_size):
                ...
                prediction = reconstruction(predictions[i], type='test')
            ...
            for i in range(conf.examples_to_show):
                ...
                pred = reconstruction(predictions[i], type='test')
                a[0][i].imshow(t)
                im = a[1][i].imshow(pred)
            plt.colorbar(im)

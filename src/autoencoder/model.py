...
class ae():
    ...
    def _init_network_params(self):
        ...
        self.weights = {
            ...
            'decoder_h3': tf.get_variable(name="decoder_w_h3",
                                          ...
                                          initializer=initializer,
                                          dtype=tf.float32),
            'decoder_h4': tf.get_variable(name="decoder_w_h6",
    ...
    def _init_encoder(self):
        with tf.variable_scope("encoder") as scope:
            ...
            self.layer_4 = tf.nn.dropout(tf.nn.relu(tf.add(tf.matmul(self.layer_3, self.weights['encoder_h4']),
                                                    self.biases['encoder_b4'])), keep_prob=self.dropout)

    # Building the decoder
    def _init_decoder(self):
        with tf.variable_scope("decoder") as scope:
            self.layer_5 = tf.nn.dropout(tf.nn.relu(tf.add(tf.matmul(self.layer_4, self.weights['decoder_h1']),
                                                    self.biases['decoder_b1'])), keep_prob=self.dropout)

            self.layer_6 = tf.nn.dropout(tf.nn.relu(tf.add(tf.matmul(self.layer_5, self.weights['decoder_h2']),
                                                    self.biases['decoder_b2'])), keep_prob=self.dropout)

            self.layer_7 = tf.nn.dropout(tf.nn.relu(tf.add(tf.matmul(self.layer_6, self.weights['decoder_h3']),
                                                    self.biases['decoder_b3'])), keep_prob=self.dropout)

            self.layer_8 = tf.nn.dropout(tf.nn.relu(tf.add(tf.matmul(self.layer_7, self.weights['decoder_h4']),
    ...
    def _init_optimizer(self):
        ...
        self.loss = tf.metrics.mean_squared_error(labels=self.y, predictions=self.y_pred)
